...
export const getReferences = async () => {
  ...
  const data = records.map(entry => ({
    sourceID: `http://localhost:3000/entries/${entry.get('sourceNode.id')}`,
    referenceType: `http://localhost:3000/referenceTypes/${
      entry.get('reference').type
    }`,
    targetID: `http://localhost:3000/entries/${entry.get('targetNode.id')}`,
  }))
  session.close()
  driver.close()
    ...
    .map(record => {
      ...
      rawReferences.forEach(({ type }, index) => {
        references[type].push(
          `http://localhost:3000/entries/${targetNodes[index].properties.id}`
        )
      })
      return {
        ...rawEntry,
        id: `http://localhost:3000/entries/${rawEntry.id}`,
